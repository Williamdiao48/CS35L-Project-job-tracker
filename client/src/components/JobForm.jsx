...
export default function JobForm({ onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ company: '', role: '', status: 'Applied', location: '', dueDate: '', outcome: 'Pending', tags: '', jobUrl: '', notes: '', salary: '', owner: '' });
  ...
  return (
    <form onSubmit={handleSubmit} style={{display:'grid',gap:8}}>
      ...
      <input name="jobUrl" placeholder="Job URL" value={form.jobUrl} onChange={handleChange} />
      <input name="salary" placeholder="Salary" value={form.salary} onChange={handleChange} />
      <input name="owner" placeholder="Owner (user id)" value={form.owner} onChange={handleChange} />
